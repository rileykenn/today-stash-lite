...
const IMAGE_BUCKET = "success-stories";
...
export default async function SuccessStoriesPage() {
  ...
  const stories = (data || []).map((story: any) => {
    // Resolve image URL on the server
  ...
  const namesFromData = Array.from(
    new Set(stories.map((s: any) => s.category_name).filter(Boolean)) as Set<string>
  );
  const ordered = CATEGORY_ORDER.filter((name) => namesFromData.includes(name));
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 pb-16 pt-8 text-[#E8FFF3]">
      ...
      <StoriesFilter stories={stories} categories={categories} />
      ...
      <section className="mt-10 rounded-2xl border border-emerald-500/40 bg-gradient-to-br from-emerald-500/20 via-emerald-500/5 to-transparent px-5 py-6 sm:px-6 sm:py-7">
        ...
        <p className="mt-2 text-sm text-emerald-50/80 max-w-xl">
          Today&apos;s Stash takes everything that worked in the coupon booklets
          and turns it into a digital, trackable membership for your town.
        </p>
