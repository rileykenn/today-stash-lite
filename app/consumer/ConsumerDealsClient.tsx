...
/**
 * Props provided by the server-side parent page.tsx
 * `initialDeals` and `initialTowns` are pre-fetched on the server
 * so Google sees real HTML content instead of "Loading Deals...".
 */
interface ConsumerDealsClientProps {
  initialDeals: Coupon[];
  initialTowns: Town[];
}

export default function ConsumerDealsClient({
  initialDeals,
  initialTowns,
}: ConsumerDealsClientProps) {
  const router = useRouter();

  // Deals & Data — seeded from server props
  const [deals, setDeals] = useState<Coupon[]>(initialDeals);
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Fetch today's redemptions for current user (if logged in)
      const { data: { session } } = await sb.auth.getSession();
      if (session && mounted) {
      ...
      // Client-side refresh of deals for fresh data (user-specific scheduling, etc.)
